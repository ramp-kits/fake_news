...
_ignore_column_names = [
    'ID', 'true_counts', 'false_counts', 'half_true_counts',
...
def _read_data(path, f_name):
    ...
    data = pd.read_csv(
        os.path.join(path, 'data', f_name), sep='\t',
        names=[
            'ID', 'label', 'statement', 'subject', 'speaker', 'job', 'state',
            'party', 'true_counts', 'false_counts', 'half_true_counts',
            'mostly_true_counts', 'pants_on_fire', 'context'])
